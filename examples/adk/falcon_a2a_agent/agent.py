...
def before_agent_setup(callback_context: CallbackContext) -> Any | None:
    ...
    if sub:
        ...
        logging.warning("No sub found in context for before_agent_setup")
    return None
...
async def jwt_interceptor_middleware(request, call_next):
    ...
    if auth_header and auth_header.startswith("Bearer "):
        ...
        try:
            # We are decoding insecurely for testing/demo purposes as per requirement (transparent to agent)
            # In production, you'd verify the signature with a public key or jwks
            ...
            if sub:
                ...
                logging.warning("No sub in token payload")
        except Exception as e:
